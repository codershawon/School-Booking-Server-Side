...
async function run() {
  try {
    ...
    const userCollection = client.db("schoolBookingDB").collection("user");
...
app.get("/", (req, res) => {
  res.send("School Booking Facilities server is running");
});

app.listen(port, () => {
  console.log(`School Booking Facilities server is running on port: ${port}`);
});
